...
// 10. POST /api/admin/users/:id/update - Update role / coins for a user
router.post("/api/admin/users/:id/update", requireAuth, async (req, res) => {
    if (!req.user.isAdmin) {
        return res.status(403).json({ error: "Access denied. Admins only." });
    }
    try {
        const { isAdmin, coins } = req.body || {};
        ...
        res.json({ success: true, user: { _id: targetUser._id, username: targetUser.username, isAdmin: targetUser.isAdmin, coins: targetUser.coins } });
    } catch (e) {
